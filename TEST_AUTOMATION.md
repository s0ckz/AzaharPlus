...
On first run, a "No Mii characters" dialog appears. Must use `input swipe` (NOT `input tap` — tap doesn't register in the emulator):
```bash
adb -s $DEVICE shell "input swipe 320 350 321 351 100"
```
Wait ~20s for the game to load through the title screen to main menu.
...
**IMPORTANT:** `input tap` does NOT work with the emulator. Always use `input swipe x y x+1 y+1 100` instead.
...
**A fix is only considered successful if the game runs for 2 HOURS at the main menu without crashing.** The main menu cycles through random course previews which triggers surface creation/destruction — the crash trigger. Any crash within 1 hour = not fixed.
