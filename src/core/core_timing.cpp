...
void Timing::Timer::Advance() {
    ...
    // Safe to touch the singleton here: Advance() is only called from the emu thread after
    // System has been constructed. The DSP HLE tick also runs inside this loop and has its
    // own Begin/EndDSPProcessing bracket — perf_stats.cpp reports core_timing exclusive
    // of dsp_hle, so the two buckets don't double-count.
